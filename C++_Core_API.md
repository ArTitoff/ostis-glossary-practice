...
уществуют стандартные макросы, которые вы можете использовать для логирования вашего кода. Они инкапсулируют класс ```ScLogger```, чтобы предотвратить неуместный доступ к нему и предоставить метапрограммирование при логировании сообщений.
...
Уровень логирования можно настроить в конфигурационном файле ```sc-machine.ini```. Измените параметр ```log_level``` в группе ```[sc-memory]``` на одно из значений из списка ```[Debug, Info, Warning, Error]```. Посмотрите пример конфигурационного файла **ССЫЛКА НА ФАЙЛ**, чтобы узнать больше о группах и их параметрах.
...
```ScType::ConstCommonArc``` — это sc-тип sc-дуги, который соединяет два sc-элемента в некотором отношении. ```ScType::ConstPermPosArc``` — это sc-тип sc-дуги, который обозначает принадлежность целевого sc-элемента к исходному sc-элементу. Sc-дуга с sc-типом ```ScType::ConstCommonArc``` между двумя sc-элементами может быть преобразована в sc-узел, к которому принадлежат эти два sc-элемента.
